...
import { BlockReasonDialog } from "@/app/(dashboard)/tasks/block-reason-dialog";
          ...
          <div className="absolute left-0 top-full z-[61] mt-1 min-w-[130px] rounded-lg border border-[#1A1A1A] bg-[#111] p-1 shadow-xl">
            {options.map((opt) => (
              ...
              >
                ...
                {opt.label}
                {opt.value === value && <Check size={10} className="ml-auto text-[#E24B4A]" />}
              </button>
            ))}
          </div>
        </>
  ...
  return (
    <div className="flex items-center justify-between py-2">
      ...
      <div className="relative">
        <button
          onClick={() => canEdit && setOpen(!open)}
          className={`text-[12px] text-[#ccc] transition-colors ${canEdit ? "hover:text-white" : ""}`}
        >
          {displayValue}
        </button>
        {open && (
          <>
            ...
            <div className="absolute right-0 top-full z-[61] mt-1 max-h-[200px] min-w-[160px] overflow-y-auto rounded-lg border border-[#1A1A1A] bg-[#111] p-1 shadow-xl">
              {options.map((opt) => (
                ...
                >
                  {opt.label}
                  {opt.value === value && <Check size={10} className="text-[#E24B4A]" />}
                </button>
              ))}
            </div>
          </>
  ...
  return (
    <div>
      <div className="group flex items-center gap-2 rounded-md px-1 py-1.5 transition-colors hover:bg-white/[0.02]">
        ...
        {/* Status pill */}
        <span
          className="rounded-full px-1.5 py-px text-[9px] font-medium uppercase"
          style={{
            background: `${STATUS_COLORS[subtask.status] ?? "#444"}15`,
        ...
        {canEdit && (
          <button
            onClick={onDelete}
            className="shrink-0 rounded p-0.5 text-[#333] opacity-0 transition-all hover:text-[#E24B4A] group-hover:opacity-100"
          >
            <Trash2 size={12} />
          </button>
        )}
      ...
      {/* Inline expand — placeholder for description */}
      {expanded && (
        <div className="ml-6 mb-1 rounded-md border border-[#141414] bg-[#0A0A0A] px-3 py-2 text-[12px] text-[#666]">
          Detalhes da subtask (em breve)
        </div>
      )}
    </div>
  );
...
function AddSubtaskInput({
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const result = await createSubtask(parentTaskId, trimmed);
      if (result.error) {
  ...
  if (!showInput) {
    return (
      <button
        ...
        className="flex items-center gap-1.5 py-1.5 text-[12px] text-[#444] transition-colors hover:text-white"
      >
        <Plus size={12} />
        {t("addSubtask")}
      </button>
  ...
  return (
    <div className="flex items-center gap-2 py-1">
      <input
        ...
        placeholder="Titulo da subtask..."
        disabled={isPending}
        className="min-w-0 flex-1 rounded-md border border-[#1A1A1A] bg-[#050505] px-2.5 py-1.5 text-[12px] text-[#ddd] placeholder-[#333] outline-none transition-colors focus:border-[#E24B4A]"
      />
      <button
        onClick={handleSubmit}
        disabled={isPending || !title.trim()}
        className="rounded-md bg-[#E24B4A] px-2.5 py-1.5 text-[11px] font-medium text-white transition-colors hover:bg-[#D4403F] disabled:opacity-50"
      >
        {isPending ? "..." : "Criar"}
      </button>
    </div>
...
export function TaskDetailDialog({
  ...
  const statusOptions = [
    { value: "backlog", label: tK("backlog"), color: STATUS_COLORS.backlog },
    { value: "doing", label: tK("doing"), color: STATUS_COLORS.doing },
    { value: "on_hold", label: tK("onHold"), color: STATUS_COLORS.on_hold },
    { value: "review", label: tK("review"), color: STATUS_COLORS.review },
    { value: "done", label: tK("done"), color: STATUS_COLORS.done },
    { value: "cancelled", label: "Cancelled", color: "#666" },
  ];

  const priorityOptions = [
    { value: "low", label: tP("low"), color: "#6B7280" },
  ...
  const getTypeLabel = (type: string) =>
    typeKeys.includes(type) ? tT(type as keyof IntlMessages["tasks"]["types"]) : type;
  ...
  function handleFieldUpdate(field: string, value: string | string[] | null) {
    ...
    startTransition(async () => {
      ...
      else await fetchData(taskId);
    });
  }

  function handleStatusChange(newStatus: string) {
    if (newStatus === "on_hold") { setBlockDialogOpen(true); return; }
  ...
  function handleResolveBlock(blockId: string) {
    startTransition(async () => {
      ...
      if (result.error) toast.error(result.error);
      else if (taskId) await fetchData(taskId);
    });
  }
  ...
  const readyToAdvance = task?.ready_to_advance === true;
      ...
      <AnimatePresence>
        {taskId && (
          <>
            ...
            />
            <motion.aside
              className="fixed inset-y-0 right-0 z-50 flex w-full flex-col sm:w-[520px]"
              style={{
                background: "var(--bg-1)",
                ...
                <div className="flex flex-1 flex-col overflow-y-auto">
                    ...
                    >
                      ...
                      {canEdit && (
                        <button
                          ...
                          disabled={isPending}
                          className="rounded-md bg-[#10B981] px-3 py-1 text-[11px] font-semibold text-white transition-colors hover:bg-[#0D9668] disabled:opacity-50"
                        >
                          {tK("done")}
                        </button>
                      )}
                  ...
                  <div className="shrink-0 px-6 pt-6">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        {/* Display ID */}
                        {task.display_id && (
                          <span className="font-mono text-[11px] text-[#555]">{task.display_id}</span>
                        )}
                        {/* Title */}
                        {editingTitle && canEdit ? (
                          <input
                            ...
                            className="mt-1 w-full border-b border-[#E24B4A] bg-transparent font-display text-[20px] font-medium text-[#eee] outline-none"
                          />
                        ) : (
                          <h2
                            onClick={() => canEdit && setEditingTitle(true)}
                            className={`mt-1 font-display text-[20px] font-medium leading-snug text-[#eee] ${
                              canEdit ? "cursor-text rounded-md transition-colors hover:bg-white/[0.03]" : ""
                            }`}
                          >
                            {task.title}
                          </h2>
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                      <DropdownPill options={statusOptions} value={task.status} onChange={handleStatusChange} canEdit={canEdit} />
                      <DropdownPill options={priorityOptions} value={task.priority} onChange={(v) => handleFieldUpdate("priority", v)} canEdit={canEdit} />
                      {typeColor && (
                        <span
                  ...
                  <div className="px-6 pt-4">
                    ...
                    <div className="mt-2 flex flex-col">
                      ...
                      />
                      <div className="flex items-center justify-between py-2">
                        <span className="text-[11px] text-[#444]">Data limite</span>
                        {canEdit ? (
                          <input
                            type="date"
                            value={task.due_date ?? ""}
                          ...
                          <span className={`text-[12px] ${
                            ...
                            {task.due_date ? format(parseISO(task.due_date), "dd MMM yyyy", { locale: ptBR }) : "—"}
                          </span>
                        )}
                      </div>
                  ...
                  {/* 4. Subtasks (only for root tasks) */}
                  {isRoot && (
                    <div className="px-6 pt-4">
                      <div className="flex items-center gap-2">
                        ...
                        {readyToAdvance && (
                          <span
                            className="ml-auto inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-medium"
                    ...
                    <div className="px-6 pt-4">
                      ...
                      <div className="mt-2 flex flex-col gap-2">
                        {blocks.map((block) => {
                          ...
                          const blockedBy = Array.isArray(block.blocked_by) ? block.blocked_by[0] : block.blocked_by;
                          const timeAgo = formatDistanceToNow(new Date(block.created_at), { addSuffix: true, locale: ptBR });
                          return (
                            <div key={block.id} className={`rounded-lg border p-3 ${block.resolved ? "border-[#111] bg-[#060606]" : "border-[#F59E0B20] bg-[#F59E0B06]"}`}>
                              <p className={`text-[12px] leading-relaxed ${block.resolved ? "text-[#444] line-through" : "text-[#ccc]"}`}>{block.reason}</p>
                              <div className="mt-1.5 flex items-center gap-2 text-[10px] text-[#444]">
                                <span>reportado por {createdBy?.name ?? "desconhecido"}</span>
                                <span>·</span>
                                <span suppressHydrationWarning>{timeAgo}</span>
                                {blockedBy?.name && (<><span>·</span><span>destravar: {blockedBy.name}</span></>)}
                              </div>
                    ...
                    {activities.length === 0 ? (
                      ...
                      <div className="mt-2">
                        {activities.map((activity, i) => {
                          const activityUser = Array.isArray(activity.user) ? activity.user[0] : activity.user;
                          ...
                          const initials = activityUser?.name
                            ? activityUser.name.split(" ").map((n: string) => n[0]).slice(0, 2).join("").toUpperCase()
                            : "?";
                          return (
                            <div key={activity.id}>
                              {i > 0 && <div className="h-px bg-[#0F0F0F]" />}
                              <div className="flex items-center gap-2.5 py-2">
                                <Avatar user={{ name: activityUser?.name ?? "Desconhecido", initials, color: "#555" }} size={20} />
                                <p className="min-w-0 flex-1 truncate text-[11px] text-[#666]">
                                  <span className="font-medium text-[#888]">{activityUser?.name ?? "Desconhecido"} </span>
                                  {getActionText(activity.action, activity.metadata)}
                                </p>
                                <span suppressHydrationWarning className="shrink-0 text-[10px] text-[#333]">{timeAgo}</span>
                              </div>
                            </div>
                          );
                        })}
                  ...
                  {/* Footer */}
                  <div className="shrink-0 border-t border-[#141414] px-6 py-4">
